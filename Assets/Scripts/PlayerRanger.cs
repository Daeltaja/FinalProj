...
public class PlayerRanger : PlayerBase {
	...
	void Attack()
	{
		if(!lockedOn)
		{
			if(currStam >= 10f)
			{
				...
				//attack
				hitDelay = 0f;
				GameObject arrowGO = Instantiate(arrow, transform.position + rollDir, transform.rotation) as GameObject;
				arrowGO.rigidbody.AddForce(rollDir * 45f, ForceMode.Impulse);
	...
	void OnGUI()
	{
		...
		GUI.DrawTexture (new Rect(Screen.width - 10, 10, -currHealth, 15), healthOver);

		GUI.DrawTexture (new Rect(Screen.width - 10, 30, -100, 15), stamUnder);
		GUI.DrawTexture (new Rect(Screen.width - 10, 30, -currStam, 15), stamOver);
	}
